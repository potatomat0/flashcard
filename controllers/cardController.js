...
// @access  Private
exports.addCardToDeck = asyncHandler(async (req, res) => {
...
// @access  Private
exports.getCardsInDeck = asyncHandler(async (req, res) => {
    const { deckId } = req.params;
...
exports.getCardById = asyncHandler(async (req, res) => {
    ...
    // Ensure the user owns the deck this card belongs to
    const deck = await Deck.findById(card.deck_id);
    if (!deck || deck.user_id.toString() !== req.user.id) {
        return res.status(401).json({ message: 'User not authorized to access this card' });
    }
...
// @access  Private
exports.submitCardReview = asyncHandler(async (req, res) => {
    const { id } = req.params;
...
// @access  Private
exports.addDefaultCardToDeck = asyncHandler(async (req, res) => {
    const { deckId } = req.params;
